...
git init
cook init
cook "Create a simple todo app as a single index.html file. It should support: add a task, mark complete, delete, and filter by status (all/active/done). No dependencies — vanilla JS only."```
...
```sh
cook "Add a dark mode toggle button to the todo app"```
...
```sh
cook "Improve the visual design of the todo app" x3```
...
```sh
cook "Polish the CSS spacing and alignment" repeat 3```
...
```sh
cook "Add keyboard shortcuts (Enter to add, Delete to remove selected)" review```
...
```sh
cook "Add drag-and-drop task reordering" review --max-iterations 5```
...
```sh
cook "Improve error handling in the app" review 5```
...
```sh
cook "Improve accessibility (ARIA labels, keyboard nav, focus styles)" x3 review```
...
```sh
cook "Refactor the JS to be cleaner and more maintainable" review x3```
...
```sh
cook "Create a file PLAN.md with exactly 3 tasks, each on its own line as `- [ ] <task>`. Tasks: (1) add task priorities (high/medium/low), (2) add a search/filter bar, (3) add localStorage persistence. Do not implement them yet."```
...
```sh
# Reset PLAN.md first
cook "Rewrite PLAN.md with the same 3 tasks but all unchecked again."
cook "Read PLAN.md. Find the first unchecked task, implement it, then mark it [x] in PLAN.md." \
...
```sh
cook "Add a task count badge to the page title" review --hide-request```
...
```sh
cook doctorcook doctor --work-model haiku --gate-agent claude```
